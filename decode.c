...
*/
/* PROGRAM TO PERFORM DECODING */
//Built in and user defined preprocessor directives
#include<stdio.h>
#include<string.h>
#include "decode.h"
#include "types.h"
...
Status do_decoding(DecodeInfo *decInfo)
{
    ...
    if (open_decode_files(decInfo) == e_success)
    {
        ...
        {
            printf("Decoded magic string is success\n");
                    
            //To check whether decoded file extension size is decoded successfully or not
            if (decode_secret_file_extn_size(decInfo) == e_success)
            {
                printf("Decoded secret file extension size is success\n");
                ...
                if(decode_secret_file_extn(decInfo -> image_data_size, decInfo) == e_success)
                {
                    ...
                    {
                        printf("Decoded file size is success\n");

                        //To check whether decoded data to file  successful or not
                        if (decode_secret_file_data(decInfo) == e_success)
                        {
                            printf("Decoded the secret data is success\n");
                            return e_success;
                        }
                        else
                        {
